...
class ProjectSync {
  ...
  createDevOpsConfig(projectName, setupBackend, setupFrontend, setupTesting, setupAgents) {
    ...
    const configFile = path.join(devopsDir, 'project-config.yml');
    const projectType = setupBackend && setupFrontend ? 'fullstack' : (setupBackend ? 'backend' : 'frontend');
    
    const config = `# Project Configuration for DevOps System
# Generated by sync-project.js on ${new Date().toISOString()}
...
features:
  backend_enabled: ${setupBackend}
  frontend_enabled: ${setupFrontend}
  testing_enabled: ${setupTesting}
  agents_enabled: ${setupAgents}
...
# DevOps commands
commands:
  qa: "./devops/ops/ops qa"
  build: "./devops/ops/ops build"
  deploy: "./devops/deploy/deploy production"
  setup: "./devops/ops/ops setup"
`;

    fs.writeFileSync(configFile, config);
    console.log('  ✅ Created DevOps project configuration');
  }
